...
Если кого-то раздражает нескрьезное поведение нейронки, то можете убрать абзац из системного промпта. Могу заметить, что при таком общении, в случае ошибок она не так сильно раздражает + работает почему-то лучше :) 
